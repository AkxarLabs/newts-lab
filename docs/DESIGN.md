...
Where Newts' Lab now stands against the systems surveyed in §2, and what remains deliberately or temporarily missing.
...
Policy: Newts' Lab stays a content-and-protocol template with stdlib+pyyaml tools; external systems are sources of *mechanisms* first, dependencies only when a piece is clean, light, and load-bearing.
