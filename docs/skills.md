...
### `/write-paper <slug>`
Figures and tables first — generated by committed scripts from run artifacts, never hand-made. Every quantitative claim gets a `claims.yaml` entry *as it enters the text*. Citations only from lit-review notes. Setup-and-results written before the narrative sections.

### `/review-paper <slug>`
Part A: mechanical claims audit (`tools/audit_claims.py`) + checks the script can't do — **blocking**. Part B: `/critique-paper` own-draft ensemble. Part C: consolidates scores/decision/action items and routes; accept + zero unrefuted vetoes → **Gate 3**. Cycle cap: `critique.max_review_cycles`.
