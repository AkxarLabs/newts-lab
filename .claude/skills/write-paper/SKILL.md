...
2. **Figures/tables first.** Write/extend scripts in the project repo's `scripts/figures/` that generate every figure and table from `runs/` artifacts; commit them in the project; copy outputs to `papers/<slug>/figures/`. No hand-made figures, no hand-typed result numbers — if a table is text, it's generated text pasted whole.
