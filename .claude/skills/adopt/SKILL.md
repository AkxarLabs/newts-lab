...
The PI may arrive with an idea, a known literature, a settled design, or a
half-finished codebase. This skill scaffolds exactly the prerequisites the entry
state needs, records what was skipped, and routes onward — so any *part* of the
workflow is usable alone.
...
- A draft paper needing the review machinery only? → enter at `internal-review`
  (studies/<slug>/paper/ + claims.yaml required first — see integrity rules).

## 2. Scaffold the prerequisites

For every entry point: create `studies/<slug>/IDEA.md` from `templates/idea/` carrying
...
| `internal-review` | `studies/<slug>/paper/` with the draft + a real `claims.yaml` (every number → artifact) | `/review-paper <slug>` |
...
2. Retrofit the project contract — copy from `templates/project/` **everything
   ...
   placeholders), `PLAN.md` (write WITH the PI: frozen eval, kill criteria, done vs planned),
   `EXPERIMENT_LOG.md` (template entry-format header),
   `NOTES.md` (seed with gotchas/dead-ends the PI already knows from the existing work),
   `configs/base.yaml` + one smoke experiment yaml adapted to their entry point, a minimal
   `pyproject.toml` if absent, an empty `runs/registry.jsonl`, and `scripts/`. Also copy
   `templates/project/.claude/` (trace hooks **and** the vendored
   `grilling`/`domain-modeling`/`grill-with-docs` skills) if absent — domain-agnostic, no
   substitution. Don't restructure their code — the contract wraps it; runs just need to land
   in `runs/<run_id>/` with a registry line (adapt `scripts/run.py` to call their entry point —
   also where the smoke yaml points).
...
- **Skipped stages are recorded, not hidden.** Each gets a line in IDEA.md's state
  log: `<stage>: PI-waived <date>` — so reviewers and future sessions know what was
  never machine-verified. A PI-waived lit review means `/write-paper`'s Related
  Work needs real `/lit-review`-style verified notes before citing — budget for it.
- **Hard rule 1 is not waivable.** Pre-existing numbers without run artifacts cannot
  enter analyses or papers. Offer the PI the honest menu: re-run under the contract
  (they become planned reproduction rows in PLAN.md), or drop them.
- Gates are not waivable: an adopted idea entering past Gate 1 needs the PI's
  approval recorded in IDEA.md at adoption time (the interview *is* that conversation
  — write it down); Gate 2/3 apply unchanged.
- Adoption never overwrites existing files; additions only, each listed in the report.
