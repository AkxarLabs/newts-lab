...
5. **Write it down:** `analysis-YYYY-MM-DD.md` in the project root (claims → run ids throughout). Distill to the hub: `FINDINGS.md` (confirmed, with evidence pointers), `FAILURES.md` (what didn't work + diagnosis), `OPEN-QUESTIONS.md` (new threads). A durable *settled-here* or *abandoned-here* conclusion also belongs in the project's `NOTES.md` (the local "don't re-derive this" cache, in addition to the hub promotion). Update registry + notebook.
