...
3. **Run** via `scripts/run.py` (long runs: background with output capture). PILOT/FULL campaigns first acquire a compute slot (hard rule 13: `tools/run_slots.py acquire <slug> exp-NNN`; release when the ledger entry is written). Stage budgets come from `control.yaml` and are enforced by the watchdog — a run that needs more budget is a PI flag, not a config tweak.
